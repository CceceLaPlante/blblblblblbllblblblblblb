...
def _minimax(state, maximizing_player_id, row, col, alpha, beta, cache,init_state):
    ...
    if state.is_terminal() :
        ret = get_ret(state)
        k = get_key(state,init_state)
        cache[k] = ret[maximizing_player_id]-ret[1-maximizing_player_id]
        return ret[maximizing_player_id]-ret[1-maximizing_player_id], cache

    inc = in_cache(state, cache,init_state)
    if inc is not None:
    ...
    if player == maximizing_player_id :
        ...
        for action in state.legal_actions() :
            val,c = _minimax(state.child(action), maximizing_player_id, row, col, alpha, beta, cache,init_state)
            cache.update(c)
...
def minimax_search ( game, 
    ...
    action_val = [_minimax(state.child(action),maximizing_player_id,row,col,alpha,beta,cache,init_state) for action in state.legal_actions()]
